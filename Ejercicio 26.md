...
```assembly
.section .data
N: .word 10            // Número N (cantidad de números naturales a sumar)
output_msg: .asciz "La suma de los primeros N números naturales es: " // Mensaje de salida
newline: .asciz "\n"    // Salto de línea
...
_start:
    // 1. Cargar el valor de N
    LDR w0, =N            // Cargar dirección de N
    LDR w0, [w0]          // Cargar el valor de N en w0

    // 2. Inicializar la suma en 0
    MOV w1, #0            // w1 = suma (inicializado a 0)

    // 3. Sumar los primeros N números naturales
    MOV w2, #1            // w2 = contador (inicializado a 1)

sum_loop:
    ADD w1, w1, w2        // w1 = w1 + w2 (suma acumulada)
    ADD w2, w2, #1        // w2 = w2 + 1 (incremento del contador)
    CMP w2, w0            // Comparar contador con N
    BLE sum_loop          // Si w2 <= N, repetir el ciclo
    ...
    // 5. Convertir el valor de la suma (w1) a ASCII y mostrarlo
    MOV w0, w1            // Mover el valor de la suma a w0
